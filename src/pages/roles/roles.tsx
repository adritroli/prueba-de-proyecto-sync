...
export default function RolesPermisosPage() {
  ...
  const fetchPermissions = async (roleId: string) => {
    try {
      const response = await fetch(
        `http://localhost:5000/api/auth/roles/${roleId}/permissions`
      );
      const data = await response.json();
  ...
  const getModulePermissions = (moduleId: number) => {
    return (
      permissions.find((p) => p.moduloId === moduleId) || {
        moduloId: moduleId,
        view: false,
        can_create: false,
        edit: false,
  ...
  const handlePermissionChange = async (
    ...
    try {
      await fetch(
        `http://localhost:5000/api/auth/roles/${selectedRole}/permissions`,
        {
          method: "PUT",
